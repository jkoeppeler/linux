...
#define NFT_MAX_SET_NELEMS                        \
	((2048 - sizeof(struct nft_trans_elem)) / \
	 sizeof(struct nft_trans_one_elem))
...
static void nft_validate_state_update(struct nft_table *table,
...
static bool nft_trans_collapse_set_elem_allowed(const struct nft_trans_elem *a,
						const struct nft_trans_elem *b)
{
	 ...
	 */
	return a->set == b->set && a->bound == b->bound &&
	       a->nelems < NFT_MAX_SET_NELEMS;
}
					...
					struct nft_trans_elem *trans)
{
	...
	new_trans =
		krealloc(tail, struct_size(tail, elems, nelems), GFP_KERNEL);
	if (!new_trans) {
				   ...
				   struct nft_trans *trans)
{
	...
	case NFT_MSG_DELSETELEM:
		return nft_trans_collapse_set_elem(
			nft_net, nft_trans_container_elem(tail),
					   ...
					   struct nft_trans *trans)
{
	...
	case NFT_MSG_NEWSET:
		...
		if (!nft_trans_set_update(trans) &&
			...
			list_add_tail(&binding->binding_list,
				      &nft_net->binding_list);

		list_add_tail(&trans_set->list_trans_newset,
			      &nft_net->commit_set_list);
		break;
	case NFT_MSG_NEWCHAIN:
...
static void nft_trans_commit_list_add_elem(struct net *net,
					   struct nft_trans *trans)
{
	struct nftables_pernet *nft_net = nft_pernet(net);
...
static struct nft_trans *nft_trans_alloc_chain(const struct nft_ctx *ctx,
					       int msg_type)
{
...
static int nf_tables_delrule_deactivate(struct nft_ctx *ctx,
					  ...
					  u8 genmask, u32 nlpid)
{
	...
	list_for_each_entry_rcu(table, &nft_net->tables, list,
		...
		if (!nla_strcmp(nla, table->name) && table->family == family &&
		    nft_active_genmask(table, genmask)) {
			if (nft_table_has_owner(table) && nlpid &&
			    table->nlpid != nlpid)
				     ...
				     int family, const struct nft_table *table)
{
	...
	nlh = nfnl_msg_put(skb, portid, seq,
			   nfnl_msg_type(NFNL_SUBSYS_NFTABLES, event), flags,
	...
	if (event == NFT_MSG_DELTABLE || event == NFT_MSG_DESTROYTABLE) {
		nlmsg_end(skb, nlh);
		return 0;
	}
	...
	if (table->udata) {
		if (nla_put(skb, NFTA_TABLE_USERDATA, table->udlen,
			    table->udata))
			goto nla_put_failure;
	}
				 ...
				 struct netlink_callback *cb)
{
	...
	list_for_each_entry_rcu(table, &nft_net->tables, list) {
		...
		if (!nft_is_active(net, table))
			continue;
		if (nf_tables_fill_table_info(
			    skb, net, NETLINK_CB(cb->skb).portid,
			    cb->nlh->nlmsg_seq, NFT_MSG_NEWTABLE, NLM_F_MULTI,
...
static int nf_tables_updtable(struct nft_ctx *ctx)
{
	...
	if ((nft_table_has_owner(ctx->table) && !(flags & NFT_TABLE_F_OWNER)) ||
	    (flags & NFT_TABLE_F_OWNER && !nft_table_is_orphan(ctx->table)))
		return -EOPNOTSUPP;
	...
	if ((flags & NFT_TABLE_F_OWNER) && !nft_table_has_owner(ctx->table)) {
			      ...
			      const struct nlattr *const nla[])
{
	...
	if (IS_ERR(table)) {
		...
		nft_ctx_init(&ctx, net, skb, info->nlh, family, table, NULL,
			     nla);
	...
	if (nla[NFTA_TABLE_USERDATA]) {
		table->udata = nla_memdup(nla[NFTA_TABLE_USERDATA],
					  GFP_KERNEL_ACCOUNT);
		if (table->udata == NULL)
...
static int nft_flush_table(struct nft_ctx *ctx)
{
	...
	list_for_each_entry_safe(flowtable, nft, &ctx->table->flowtables,
				 list) {
			      ...
			      const struct nlattr *const nla[])
{
	...
	if (info->nlh->nlmsg_flags & NLM_F_NONREC && table->use > 0)
		return -EBUSY;
					  ...
					  const struct nlattr *nla, u8 genmask)
{
	...
	WARN_ON(!rcu_read_lock_held() && !lockdep_commit_lock_is_held(net));
...
static const struct nla_policy nft_hook_policy[NFTA_HOOK_MAX + 1] = {
	...
	[NFTA_HOOK_DEV] = { .type = NLA_STRING, .len = IFNAMSIZ - 1 },
};
...
static int nft_dump_basechain_hook(struct sk_buff *skb, const struct net *net,
				   int family,
				     ...
				     const struct list_head *hook_list)
{
	...
	if (!hook_list &&
	    (event == NFT_MSG_DELCHAIN || event == NFT_MSG_DESTROYCHAIN)) {
		nlmsg_end(skb, nlh);
		return 0;
	}
	...
	list_for_each_entry_rcu(table, &nft_net->tables, list) {
		...
		list_for_each_entry_rcu(chain, &table->chains, list) {
			...
			if (!nft_is_active(net, chain))
				continue;
			if (nf_tables_fill_chain_info(
				    skb, net, NETLINK_CB(cb->skb).portid,
				    cb->nlh->nlmsg_seq, NFT_MSG_NEWCHAIN,
				    NLM_F_MULTI, table->family, table, chain,
				    NULL) < 0)
				goto done;
...
static void nft_chain_stats_replace(struct nft_trans_chain *trans)
{
	...
	trans->stats = rcu_replace_pointer(chain->stats, trans->stats,
...
nft_netdev_hook_alloc(struct net *net, const struct nlattr *attr, bool prefix)
{
	struct nf_hook_ops *ops;
				  ...
				  struct netlink_ext_ack *extack, u32 flags)
{
	...
	if (flags & NFT_CHAIN_HW_OFFLOAD && list_empty(hook_list))
		return -EINVAL;
				...
				u32 flags, struct netlink_ext_ack *extack)
{
	...
	err = nla_parse_nested_deprecated(
		ha, NFTA_HOOK_MAX, nla[NFTA_CHAIN_HOOK], nft_hook_policy, NULL);
	if (err < 0)
		return err;

	if (!basechain) {
		if (!ha[NFTA_HOOK_HOOKNUM] || !ha[NFTA_HOOK_PRIORITY]) {
			NL_SET_BAD_ATTR(extack, nla[NFTA_CHAIN_NAME]);
			return -ENOENT;
		}
		...
		if (nla[NFTA_CHAIN_TYPE]) {
			type = nf_tables_chain_type_lookup(
				net, nla[NFTA_CHAIN_TYPE], family, true);
			if (IS_ERR(type)) {
				NL_SET_BAD_ATTR(extack, nla[NFTA_CHAIN_TYPE]);
				return PTR_ERR(type);
			}
		}
		if (hook->num >= NFT_MAX_HOOKS ||
		    !(type->hook_mask & (1 << hook->num)))
			return -EOPNOTSUPP;
		...
		if (nla[NFTA_CHAIN_TYPE]) {
			type = __nf_tables_chain_type_lookup(
				nla[NFTA_CHAIN_TYPE], family);
			if (!type) {
				NL_SET_BAD_ATTR(extack, nla[NFTA_CHAIN_TYPE]);
				return -ENOENT;
			}
...
nf_tables_chain_alloc_rules(const struct nft_chain *chain, unsigned int alloc)
{
	if (alloc > INT_MAX)
		return NULL;
	...
	if (nft_base_chain_netdev(family, hook->num)) {
		...
		list_for_each_entry(h, &basechain->hook_list, list) {
			list_for_each_entry(ops, &h->ops_list, list)
				nft_basechain_hook_init(ops, family, hook,
							chain);
		}
	}
			      ...
			      u32 flags, struct netlink_ext_ack *extack)
{
	...
	if (nla[NFTA_CHAIN_NAME]) {
		chain->name =
			nla_strdup(nla[NFTA_CHAIN_NAME], GFP_KERNEL_ACCOUNT);
	} else {
		if (!(flags & NFT_CHAIN_BINDING)) {
			err = -EINVAL;
	...
	if (nla[NFTA_CHAIN_USERDATA]) {
		chain->udata = nla_memdup(nla[NFTA_CHAIN_USERDATA],
					  GFP_KERNEL_ACCOUNT);
		if (chain->udata == NULL) {
			err = -ENOMEM;
			goto err_destroy_chain;
			      ...
			      struct netlink_ext_ack *extack)
{
	...
	if (nla[NFTA_CHAIN_HOOK]) {
		...
		if (nft_base_chain_netdev(ctx->family,
					  basechain->ops.hooknum)) {
			list_for_each_entry_safe(h, next, &hook.list, list) {
				list_for_each_entry(ops, &h->ops_list, list) {
				...
				if (nft_hook_list_find(&basechain->hook_list,
						       h)) {
					list_del(&h->list);
					nft_netdev_hook_free(h);
					continue;
				...
				nft_net = nft_pernet(ctx->net);
				list_for_each_entry(
					trans, &nft_net->commit_list, list) {
					if (trans->msg_type !=
						    NFT_MSG_NEWCHAIN ||
					    trans->table != ctx->table ||
					    !nft_trans_chain_update(trans))
	...
	if (nla[NFTA_CHAIN_HANDLE] && nla[NFTA_CHAIN_NAME]) {
		...
		chain2 = nft_chain_lookup(ctx->net, table, nla[NFTA_CHAIN_NAME],
					  genmask);
		if (!IS_ERR(chain2)) {
			NL_SET_BAD_ATTR(extack, nla[NFTA_CHAIN_NAME]);
			err = -EEXIST;
	...
	if (table->flags & __NFT_TABLE_F_UPDATE && !list_empty(&hook.list)) {
		NL_SET_BAD_ATTR(extack, attr);
		err = -EOPNOTSUPP;
		goto err_hooks;
	}
			      ...
			      const struct nlattr *const nla[])
{
	...
	if (nla[NFTA_CHAIN_HOOK]) {
		if (NFNL_MSG_TYPE(info->nlh->nlmsg_type) ==
			    NFT_MSG_DESTROYCHAIN ||
		...
		if (nft_is_base_chain(chain)) {
			...
			if (nft_base_chain_netdev(table->family,
						  basechain->ops.hooknum))
				return nft_delchain_hook(&ctx, basechain,
							 extack);
		}
	}

	if (info->nlh->nlmsg_flags & NLM_F_NONREC && chain->use > 0)
		return -EBUSY;
...
static int nft_expr_type_request_module(struct net *net, u8 family,
					struct nlattr *nla)
{
	if (nft_request_module(net, "nft-expr-%u-%.*s", family, nla_len(nla),
...
static const struct nft_expr_type *nft_expr_type_get(struct net *net, u8 family,
						     struct nlattr *nla)
{
	...
	if (type == NULL) {
		...
		if (nft_request_module(net, "nft-expr-%.*s", nla_len(nla),
				       (char *)nla_data(nla)) == -EAGAIN)
			return ERR_PTR(-EAGAIN);
	}
#endif
				    ...
				    const struct nft_expr *expr, bool reset)
{
	...
	if (expr->ops->dump) {
		struct nlattr *data =
			nla_nest_start_noflag(skb, NFTA_EXPR_DATA);
		if (data == NULL)
			goto nla_put_failure;
...
#ifdef CONFIG_MODULES
			if (err == -EAGAIN)
				if (nft_expr_type_request_module(
					    ctx->net, ctx->family,
			     ...
			     struct nft_expr *expr)
{
	...
	expr->ops = ops;
	if (ops->init) {
		err = ops->init(ctx, expr,
				(const struct nlattr **)expr_info->tb);
		if (err < 0)
			goto err1;
	}
				 ...
				 unsigned int base_seq, unsigned int nentries)
{
	...
	audit_log_nfcfg(buf, table->family, nentries, AUDIT_NFT_OP_RULE_RESET,
			GFP_ATOMIC);
	kfree(buf);
}
				  ...
				  const struct nft_chain *chain)
{
	...
	rules = rcu_dereference(r->r[chain->hook_num]);
	if (!rules) {
	...
	list_for_each_entry_rcu(rule, &chain->rules, list) {
		...
		if (nf_tables_fill_rule_info(
			    skb, net, NETLINK_CB(cb->skb).portid,
			    cb->nlh->nlmsg_seq, NFT_MSG_NEWRULE,
			    NLM_F_MULTI | NLM_F_APPEND, table->family, table,
			    chain, rule, handle, ctx->reset) < 0) {
			ret = 1;
			break;
		}
		entries++;
	...
	list_for_each_entry_rcu(table, &nft_net->tables, list) {
		...
		if (ctx->table && ctx->chain) {
			...
			rhl_for_each_entry_rcu(chain, tmp, list, rhlhead) {
				if (!nft_is_active(net, chain))
					continue;
				__nf_tables_dump_rules(skb, &idx, cb, table,
						       chain);
				break;
			}
			goto done;
			 ...
			 const struct nlattr *const nla[], bool reset)
{
	...
	err = nf_tables_fill_rule_info(skb2, net, portid, info->nlh->nlmsg_seq,
				       NFT_MSG_NEWRULE, 0, family, table, chain,
				       rule, 0, reset);
	if (err < 0) {
		kfree_skb(skb2);
		return ERR_PTR(err);
				   ...
				   const struct nlattr *const nla[])
{
	...
	buf = kasprintf(GFP_ATOMIC, "%.*s:%u", nla_len(nla[NFTA_RULE_TABLE]),
...
int nft_set_catchall_validate(const struct nft_ctx *ctx, struct nft_set *set)
{
	...
	list_for_each_entry_rcu(catchall, &set->catchall_list, list,
		...
		ret = nft_setelem_validate(ctx, set, &dummy_iter,
					   catchall->elem);
		if (ret < 0)
			return ret;
	}
			     ...
			     const struct nlattr *const nla[])
{
	...
	if (nla[NFTA_RULE_CHAIN]) {
		...
		chain = nft_chain_lookup_byid(net, table,
					      nla[NFTA_RULE_CHAIN_ID], genmask);
		if (IS_ERR(chain)) {
	...
	if (nla[NFTA_RULE_HANDLE]) {
		...
		if (nla[NFTA_RULE_POSITION]) {
			pos_handle = be64_to_cpu(
				nla_get_be64(nla[NFTA_RULE_POSITION]));
			...
			old_rule = nft_rule_lookup_byid(
				net, chain, nla[NFTA_RULE_POSITION_ID]);
			if (IS_ERR(old_rule)) {
				NL_SET_BAD_ATTR(extack,
	...
	if (nla[NFTA_RULE_PRIORITY]) {
		rule->priority =
			be32_to_cpu(nla_get_be32(nla[NFTA_RULE_PRIORITY]));
	} else {
	...
	for (i = 0; i < n; i++) {
		if (expr_info[i].ops) {
			...
			if (expr_info[i].ops->type->release_ops)
				expr_info[i].ops->type->release_ops(
					expr_info[i].ops);
		}
	}
	kvfree(expr_info);
			     ...
			     const struct nlattr *const nla[])
{
	...
	if (chain) {
		if (nla[NFTA_RULE_HANDLE]) {
			rule = nft_rule_lookup(info->net, chain,
			...
			rule = nft_rule_lookup_byid(net, chain,
						    nla[NFTA_RULE_ID]);
			if (IS_ERR(rule)) {
...
static const struct nft_set_type *nft_set_types[] = {
	&nft_set_hash_fast_type,   &nft_set_hash_type,	 &nft_set_rhash_type,
	&nft_set_bitmap_type,	   &nft_set_rbtree_type,
...
#define NFT_SET_FEATURES                                                     \
	(NFT_SET_INTERVAL | NFT_SET_MAP | NFT_SET_TIMEOUT | NFT_SET_OBJECT | \
	 NFT_SET_EVAL)
	...
	for (i = 0; i < ARRAY_SIZE(nft_set_types); i++) {
		...
		case NFT_SET_POL_PERFORMANCE:
			if (est.lookup < best.lookup)
				break;
			if (est.lookup == best.lookup && est.space < best.space)
				break;
			continue;
...
static const struct nla_policy nft_set_policy[NFTA_SET_MAX + 1] = {
	[NFTA_SET_TABLE] = { .type = NLA_STRING,
			     .len = NFT_TABLE_MAXNAMELEN - 1 },
	[NFTA_SET_NAME] = { .type = NLA_STRING, .len = NFT_SET_MAXNAMELEN - 1 },
					   ...
					   const struct nlattr *nla, u8 genmask)
{
	...
	list_for_each_entry_reverse(trans, &nft_net->commit_set_list,
				    list_trans_newset) {
		struct nft_set *set = trans->set;

		if (id == trans->set_id && set->table == table &&
		    nft_active_genmask(set, genmask))
			return set;
	}
	return ERR_PTR(-ENOENT);
...
static noinline_for_stack int nf_tables_fill_set_info(struct sk_buff *skb,
						      const struct nft_set *set)
{
	unsigned int nelems;
	char str[40];
			      ...
			      const struct nft_set *set, u16 event, u16 flags)
{
	...
	if (event == NFT_MSG_DELSET || event == NFT_MSG_DESTROYSET) {
		nlmsg_end(skb, nlh);
		return 0;
	}
	...
	if (timeout &&
	    nla_put_be64(skb, NFTA_SET_TIMEOUT, nf_jiffies64_to_msecs(timeout),
			 NFTA_SET_PAD))
		goto nla_put_failure;
	if (gc_int && nla_put_be32(skb, NFTA_SET_GC_INTERVAL, htonl(gc_int)))
		goto nla_put_failure;
	...
	if (set->field_count > 1 && nf_tables_fill_set_concat(skb, set))
		goto nla_put_failure;
	...
	if (set->num_exprs == 1) {
		...
		for (i = 0; i < set->num_exprs; i++) {
			if (nft_expr_dump(skb, NFTA_LIST_ELEM, set->exprs[i],
					  false) < 0)
				goto nla_put_failure;
		}
		nla_nest_end(skb, nest);
			    ...
			    struct nft_expr *exprs[], u32 num_exprs, u32 flags)
{
	...
	if (set->ktype != desc->ktype || set->dtype != desc->dtype ||
	    set->flags != flags || set->klen != desc->klen ||
	    set->dlen != desc->dlen || set->field_count != desc->field_count ||
			    ...
			    const struct nlattr *const nla[])
{
	...
	if (nla[NFTA_SET_KEY_TYPE] != NULL) {
		...
		if ((desc.ktype & NFT_DATA_RESERVED_MASK) ==
	...
	if (nla[NFTA_SET_FLAGS] != NULL) {
		...
		if ((flags & (NFT_SET_EVAL | NFT_SET_OBJECT)) ==
			...
			return -EOPNOTSUPP;
		if ((flags &
		     (NFT_SET_ANONYMOUS | NFT_SET_TIMEOUT | NFT_SET_EVAL)) ==
	...
	if (nla[NFTA_SET_DATA_TYPE] != NULL) {
		...
		if ((desc.dtype & NFT_DATA_RESERVED_MASK) ==
			    NFT_DATA_RESERVED_MASK &&
	...
	if (IS_ERR(set)) {
		...
		err = nft_set_expr_alloc(&ctx, set, nla, exprs, &num_exprs,
					 flags);
		if (err < 0)
			return err;
			    ...
			    const struct nlattr *const nla[])
{
	...
	if (set->use || (info->nlh->nlmsg_flags & NLM_F_NONREC &&
...
static int nft_mapelem_activate(const struct nft_ctx *ctx, struct nft_set *set,
				const struct nft_set_iter *iter,
				struct nft_elem_priv *elem_priv)
{
	struct nft_set_ext *ext = nft_set_elem_ext(set, elem_priv);
...
static int nft_set_elem_expr_dump(struct sk_buff *skb,
				  const struct nft_set *set,
				  const struct nft_set_ext *ext, bool reset)
{
	...
	nft_setelem_expr_foreach(expr, elem_expr, size) num_exprs++;
				  ...
				  bool reset)
{
	...
	if (nft_set_ext_exists(ext, NFT_SET_EXT_TIMEOUT)) {
		...
		if (timeout > 0) {
			...
			expires =
				READ_ONCE(nft_set_ext_timeout(ext)->expiration);
	...
	if (nft_set_ext_exists(ext, NFT_SET_EXT_USERDATA)) {
		...
		if (nla_put(skb, NFTA_SET_ELEM_USERDATA, udata->len + 1,
			    udata->data))
			goto nla_put_failure;
	}
...
static int nf_tables_dump_set(struct sk_buff *skb, struct netlink_callback *cb)
{
	...
	nlh = nfnl_msg_put(skb, portid, seq, event, NLM_F_MULTI, table->family,
				 ...
				 const struct nlattr *const nla[], bool reset)
{
	...
	nft_ctx_init(&dump_ctx->ctx, net, skb, info->nlh, family, table, NULL,
		     nla);
	dump_ctx->set = set;
				...
				const struct nlattr *const nla[])
{
	...
	nla_for_each_nested(attr, nla[NFTA_SET_ELEM_LIST_ELEMENTS], rem) {
		err = nft_get_set_elem(&dump_ctx.ctx, dump_ctx.set, attr,
				       false);
		if (err < 0) {
			NL_SET_BAD_ATTR(extack, attr);
			break;
...
static struct nft_trans *nft_trans_elem_alloc(const struct nft_ctx *ctx,
					      int msg_type, struct nft_set *set)
{
...
static int nft_set_ext_check(const struct nft_set_ext_tmpl *tmpl, u8 id,
			     u32 len)
{
	len += nft_set_ext_types[id].len;
	if (len > tmpl->ext_len[id] || len > U8_MAX)
		return -1;
					...
					u64 expiration, gfp_t gfp)
{
	...
	if (nft_set_ext_exists(ext, NFT_SET_EXT_KEY) &&
	    nft_set_ext_memcpy(tmpl, NFT_SET_EXT_KEY, nft_set_ext_key(ext), key,
			       set->klen) < 0)
		goto err_ext_check;

	if (nft_set_ext_exists(ext, NFT_SET_EXT_KEY_END) &&
	    nft_set_ext_memcpy(tmpl, NFT_SET_EXT_KEY_END,
			       nft_set_ext_key_end(ext), key_end,
			       set->klen) < 0)
		goto err_ext_check;

	if (nft_set_ext_exists(ext, NFT_SET_EXT_DATA) &&
	    nft_set_ext_memcpy(tmpl, NFT_SET_EXT_DATA, nft_set_ext_data(ext),
			       data, set->dlen) < 0)
		goto err_ext_check;
	...
	if (nft_set_ext_exists(ext, NFT_SET_EXT_TIMEOUT)) {
		...
		nft_set_ext_timeout(ext)->expiration =
...
static void nft_trans_set_elem_destroy(const struct nft_ctx *ctx,
				       struct nft_trans_elem *te)
{
	int i;
...
static int nft_set_elem_expr_setup(struct nft_ctx *ctx,
				   ...
				   struct nft_expr *expr_array[], u32 num_exprs)
{
					    ...
					    const struct nft_set *set)
{
	...
	list_for_each_entry_rcu(catchall, &set->catchall_list, list) {
		ext = nft_set_elem_ext(set, catchall->elem);
		if (nft_set_elem_active(ext, genmask) &&
		    !nft_set_elem_expired(ext) && !nft_set_elem_is_dead(ext))
			return ext;
	}
...
static int nft_setelem_insert(const struct net *net, struct nft_set *set,
			      const struct nft_set_elem *elem,
			      struct nft_elem_priv **elem_priv,
			      unsigned int flags)
{
				  ...
				  const struct nft_trans_one_elem *elem)
{
	...
	if (update->flags & NFT_TRANS_UPD_EXPIRATION)
		WRITE_ONCE(nft_set_ext_timeout(ext)->expiration,
			   get_jiffies_64() + update->expiration);
}
...
static int __nft_setelem_deactivate(const struct net *net, struct nft_set *set,
				    struct nft_set_elem *elem)
{
	void *priv;
...
static int nft_setelem_deactivate(const struct net *net, struct nft_set *set,
				  struct nft_set_elem *elem, u32 flags)
{
	int ret;
...
static void nft_setelem_remove(const struct net *net, const struct nft_set *set,
			       struct nft_elem_priv *elem_priv)
{
	if (nft_setelem_is_catchall(set, elem_priv))
				   ...
				   const struct nft_trans_elem *te)
{
	...
	for (i = 0; i < te->nelems; i++) {
		...
		nf_tables_setelem_notify(ctx, te->set, te->elems[i].priv,
				      ...
				      struct nlattr **nla, u32 flags)
{
	if ((set->flags & (NFT_SET_CONCAT | NFT_SET_INTERVAL)) ==
		...
		if (nla[NFTA_SET_ELEM_KEY_END] && flags & NFT_SET_ELEM_CATCHALL)
			return false;
	} else {
		if (nla[NFTA_SET_ELEM_KEY_END])
			return false;
			    ...
			    const struct nlattr *attr, u32 nlmsg_flags)
{
	...
	if ((flags & NFT_SET_ELEM_INTERVAL_END) &&
	    (nla[NFTA_SET_ELEM_DATA] || nla[NFTA_SET_ELEM_OBJREF] ||
	     ...
	     nla[NFTA_SET_ELEM_KEY_END] || nla[NFTA_SET_ELEM_EXPRESSIONS]))
		return -EINVAL;
	...
	} else if (set->num_exprs > 0 && !(flags & NFT_SET_ELEM_INTERVAL_END)) {
	...
	if (nla[NFTA_SET_ELEM_KEY_END]) {
		...
		err = nft_set_ext_add_length(&tmpl, NFT_SET_EXT_KEY_END,
					     set->klen);
		if (err < 0)
	...
	if (nla[NFTA_SET_ELEM_USERDATA] != NULL) {
		...
		if (ulen > 0) {
			err = nft_set_ext_add_length(
				&tmpl, NFT_SET_EXT_USERDATA, ulen);
			if (err < 0)
				goto err_parse_data;
		}
	}
	...
	if (err) {
		if (err == -EEXIST) {
			...
			if ((nft_set_ext_exists(ext, NFT_SET_EXT_DATA) &&
				...
				err = 0;
				if (nft_set_ext_exists(ext2,
						       NFT_SET_EXT_TIMEOUT)) {
					struct nft_elem_update update = {};

					if (timeout !=
					    nft_set_ext_timeout(ext2)->timeout) {
						update.timeout = timeout;
						if (expiration == 0)
							expiration = timeout;

						update.flags |=
							NFT_TRANS_UPD_TIMEOUT;
					}
					if (expiration) {
						update.expiration = expiration;
						update.flags |=
							NFT_TRANS_UPD_EXPIRATION;
					}

					if (update.flags) {
						struct nft_trans_one_elem *ue;

						ue = &nft_trans_container_elem(
							      trans)
							      ->elems[0];
						...
						ue->priv = elem_priv;
						nft_trans_commit_list_add_elem(
							ctx->net, trans);
						goto err_elem_free;
					}
				}
					  ...
					  const struct nft_trans_elem *te)
{
	...
	for (i = 0; i < te->nelems; i++) {
		if (!nft_setelem_active_next(ctx->net, te->set,
					     te->elems[i].priv)) {
			nft_setelem_data_activate(ctx->net, te->set,
						  te->elems[i].priv);
			nft_setelem_activate(ctx->net, te->set,
					     te->elems[i].priv);
		}
				...
				const struct nlattr *const nla[])
{
	...
	nla_for_each_nested(attr, nla[NFTA_SET_ELEM_LIST_ELEMENTS], rem) {
		...
		if (err == -ENOENT && NFNL_MSG_TYPE(info->nlh->nlmsg_type) ==
...
static const struct nla_policy nft_obj_policy[NFTA_OBJ_MAX + 1] = {
	[NFTA_OBJ_TABLE] = { .type = NLA_STRING,
			     .len = NFT_TABLE_MAXNAMELEN - 1 },
	[NFTA_OBJ_NAME] = { .type = NLA_STRING, .len = NFT_OBJ_MAXNAMELEN - 1 },
...
static const struct nft_object_type *__nft_obj_type_get(u32 objtype, u8 family)
{
	...
	list_for_each_entry_rcu(type, &nf_tables_objects, list) {
		if (type->family != NFPROTO_UNSPEC && type->family != family)
			continue;
			    ...
			    const struct nlattr *const nla[])
{
	...
	if (!nla[NFTA_OBJ_TYPE] || !nla[NFTA_OBJ_NAME] || !nla[NFTA_OBJ_DATA])
		return -EINVAL;
	...
	if (nla[NFTA_OBJ_USERDATA]) {
		obj->udata =
			nla_memdup(nla[NFTA_OBJ_USERDATA], GFP_KERNEL_ACCOUNT);
		if (obj->udata == NULL)
			goto err_userdata;
				   ...
				   struct nft_object *obj, bool reset)
{
	...
	if (event == NFT_MSG_DELOBJ || event == NFT_MSG_DESTROYOBJ) {
		nlmsg_end(skb, nlh);
		return 0;
	}
				...
				unsigned int base_seq, unsigned int nentries)
{
	...
	audit_log_nfcfg(buf, table->family, nentries, AUDIT_NFT_OP_OBJ_RESET,
			GFP_ATOMIC);
	kfree(buf);
}
	...
	list_for_each_entry_rcu(table, &nft_net->tables, list) {
		...
		list_for_each_entry_rcu(obj, &table->objects, list) {
			...
			rc = nf_tables_fill_obj_info(
				skb, net, NETLINK_CB(cb->skb).portid,
				cb->nlh->nlmsg_seq, NFT_MSG_NEWOBJ,
				NLM_F_MULTI | NLM_F_APPEND, table->family,
				table, obj, ctx->reset);
			if (rc < 0)
				break;
...
/* Caller must hold rcu read lock or transaction mutex */
static struct sk_buff *nf_tables_getobj_single(u32 portid,
					       const struct nfnl_info *info,
					       const struct nlattr *const nla[],
					       bool reset)
{
	...
	if (!nla[NFTA_OBJ_NAME] || !nla[NFTA_OBJ_TYPE])
		return ERR_PTR(-EINVAL);
	...
	err = nf_tables_fill_obj_info(skb2, net, portid, info->nlh->nlmsg_seq,
				      NFT_MSG_NEWOBJ, 0, family, table, obj,
				      reset);
	if (err < 0) {
		kfree_skb(skb2);
		return ERR_PTR(err);
				  ...
				  const struct nlattr *const nla[])
{
	...
	buf = kasprintf(GFP_ATOMIC, "%.*s:%u", nla_len(nla[NFTA_OBJ_TABLE]),
...
static void __nft_obj_notify(struct net *net, const struct nft_table *table,
...
{
	char *buf = kasprintf(gfp, "%s:%u", table->name, nft_base_seq(net));

	audit_log_nfcfg(buf, family, obj->handle,
			event == NFT_MSG_NEWOBJ ? AUDIT_NFT_OP_OBJ_REGISTER :
						  AUDIT_NFT_OP_OBJ_UNREGISTER,
			gfp);
	kfree(buf);

	__nft_obj_notify(net, table, obj, portid, seq, event, flags, family,
			 report, gfp);
}
...
{
	__nft_obj_notify(ctx->net, ctx->table, obj, ctx->portid, ctx->seq,
			 event, ctx->flags, ctx->family, ctx->report,
			 GFP_KERNEL);
}
...
static const struct nla_policy
	nft_flowtable_hook_policy[NFTA_FLOWTABLE_HOOK_MAX + 1] = {
				    ...
				    struct netlink_ext_ack *extack, bool add)
{
	...
	if (add) {
		...
		priority =
			ntohl(nla_get_be32(tb[NFTA_FLOWTABLE_HOOK_PRIORITY]));
		...
		if (tb[NFTA_FLOWTABLE_HOOK_NUM]) {
			hooknum = ntohl(
				nla_get_be32(tb[NFTA_FLOWTABLE_HOOK_NUM]));
		...
		if (tb[NFTA_FLOWTABLE_HOOK_PRIORITY]) {
			priority = ntohl(
				nla_get_be32(tb[NFTA_FLOWTABLE_HOOK_PRIORITY]));
...
#ifdef CONFIG_MODULES
	if (type == NULL) {
		if (nft_request_module(net, "nf-flowtable-%u", family) ==
		    -EAGAIN)
			return ERR_PTR(-EAGAIN);
	}
#endif
...
static void __nft_unregister_flowtable_net_hooks(
	struct net *net, struct nft_flowtable *flowtable,
	struct list_head *hook_list, bool release_netdev)
{
	struct nft_hook *hook, *next;
	struct nf_hook_ops *ops;
				      ...
				      struct nf_hook_ops *ops)
{
	...
	err = flowtable->data.type->setup(&flowtable->data, ops->dev,
					  FLOW_BLOCK_BIND);
	if (err < 0)
		return err;
				...
				struct netlink_ext_ack *extack)
{
	...
	list_for_each_entry_safe(hook, next, &flowtable_hook.list, list) {
		...
		list_for_each_entry(trans, &nft_net->commit_list, list) {
			...
			if (nft_hook_list_find(
				    &nft_trans_flowtable_hooks(trans), hook)) {
				err = -EEXIST;
				  ...
				  const struct nlattr *const nla[])
{
	...
	if (!nla[NFTA_FLOWTABLE_TABLE] || !nla[NFTA_FLOWTABLE_NAME] ||
	    !nla[NFTA_FLOWTABLE_HOOK])
		return -EINVAL;
	...
	flowtable->name =
		nla_strdup(nla[NFTA_FLOWTABLE_NAME], GFP_KERNEL_ACCOUNT);
	if (!flowtable->name) {
		err = -ENOMEM;
		goto err1;
	}
	...
	err = nft_register_flowtable_net_hooks(
		ctx.net, table, &flowtable->hook_list, flowtable);
	if (err < 0)
				  ...
				  const struct nlattr *const nla[])
{
	...
	if (!nla[NFTA_FLOWTABLE_TABLE] ||
	    (!nla[NFTA_FLOWTABLE_NAME] && !nla[NFTA_FLOWTABLE_HANDLE]))
		return -EINVAL;
					 ...
					 struct list_head *hook_list)
{
	...
	if (nla_put_string(skb, NFTA_FLOWTABLE_TABLE, flowtable->table->name) ||
	    ...
	    nla_put_be64(skb, NFTA_FLOWTABLE_HANDLE,
			 cpu_to_be64(flowtable->handle), NFTA_FLOWTABLE_PAD))
		goto nla_put_failure;

	if (!hook_list && (event == NFT_MSG_DELFLOWTABLE ||
	...
	list_for_each_entry_rcu(table, &nft_net->tables, list) {
		...
		list_for_each_entry_rcu(flowtable, &table->flowtables, list) {
			...
			if (nf_tables_fill_flowtable_info(
				    skb, net, NETLINK_CB(cb->skb).portid,
				    cb->nlh->nlmsg_seq, NFT_MSG_NEWFLOWTABLE,
				    NLM_F_MULTI | NLM_F_APPEND, table->family,
				    flowtable, NULL) < 0)
				goto done;
...
static int nf_tables_dump_flowtable_start(struct netlink_callback *cb)
{
	...
	if (nla[NFTA_FLOWTABLE_TABLE]) {
		...
		filter->table =
			nla_strdup(nla[NFTA_FLOWTABLE_TABLE], GFP_ATOMIC);
		if (!filter->table) {
			kfree(filter);
			return -ENOMEM;
		}
				       ...
				       struct list_head *hook_list, int event)
{
	...
	err = nf_tables_fill_flowtable_info(skb, ctx->net, ctx->portid,
					    ctx->seq, event, flags, ctx->family,
					    flowtable, hook_list);
	if (err < 0) {
		kfree_skb(skb);
		goto err;
				   ...
				   u32 portid, u32 seq)
{
	...
	nlh = nfnl_msg_put(skb, portid, seq, event, 0, AF_UNSPEC, NFNETLINK_V0,
			   nft_base_seq_be16(net));
	if (!nlh)
		goto nla_put_failure;
	...
	if (nla_put_be32(skb, NFTA_GEN_ID, htonl(nft_base_seq(net))) ||
	    ...
	    nla_put_string(skb, NFTA_GEN_PROC_NAME,
			       ...
			       struct nft_flowtable *flowtable, bool changename)
{
	...
	list_for_each_entry(hook, &flowtable->hook_list, list) {
		...
		case NETDEV_UNREGISTER:
			...
			nft_unregister_flowtable_ops(dev_net(dev), flowtable,
						     ops);
		...
		case NETDEV_REGISTER:
			...
			ops->hook_ops_type = NF_HOOK_OP_NFT_FT;
			ops->dev = dev;
			if (nft_register_flowtable_ops(dev_net(dev), flowtable,
						       ops)) {
				kfree(ops);
				return 1;
			}
...
static int __nf_tables_flowtable_event(unsigned long event,
				       struct net_device *dev, bool changename)
{
...
#if 0
#ifdef CONFIG_SAL_DEBUG
static int nf_tables_fill_travnode_info(struct sk_buff *skb,
					struct nft_chain *chain, u32 portid,
					u32 seq)
{
	struct nlmsghdr *nlh;
	...
	printk("travnodes = %llu pkts = %llu expr=%llu", trav_nodes, pkts,
	       expr);
	if (pkts == 0) {
		avg_trav_nodes = 0;
		avg_swaps = 0;
...
static int nf_tables_gettravnodes(struct sk_buff *skb,
				  const struct nfnl_info *info,
				  const struct nlattr *const nla[])
{
	struct nft_table *table;
	...
	err = nf_tables_fill_travnode_info(skb2, chain, NETLINK_CB(skb).portid,
					   nlh->nlmsg_seq);
	if (err < 0)
...
#ifdef CONFIG_SAL_GENERAL
static int nf_tables_rule_sort(void *priv, const struct list_head *a,
...
static int nf_tables_fill_resetchain_info(struct sk_buff *skb,
					  struct nft_chain *chain, u32 portid,
					  u32 seq)
{
	struct nlmsghdr *nlh;
				...
				struct netlink_ext_ack *extack)
{
	...
	err = nf_tables_fill_resetchain_info(
		skb2, chain, NETLINK_CB(skb).portid, nlh->nlmsg_seq);
...
static const struct nfnl_callback nf_tables_cb[NFT_MSG_MAX] = {
	...
	[NFT_MSG_DESTROYFLOWTABLE] = {
		...
		.call = nf_tables_gettravnodes,
		.attr_count	= NFTA_CHAIN_MAX,
		.policy		= nft_chain_policy,
	},
	[NFT_MSG_RESETCHAIN] = {
		.call = nf_tables_resetchain,
		.attr_count	= NFTA_CHAIN_MAX,
		.policy		= nft_chain_policy,
	},
#endif
...
static void nft_chain_commit_update(struct nft_trans_chain *trans)
{
	...
	if (trans->name) {
		rhltable_remove(&table->chains_ht, &trans->chain->rhlhead,
				nft_chain_ht_params);
		swap(trans->chain->name, trans->name);
		rhltable_insert_key(&table->chains_ht, trans->chain->name,
				    &trans->chain->rhlhead,
				    nft_chain_ht_params);
	}
...
{
	struct nftables_pernet *nft_net =
		container_of(w, struct nftables_pernet, destroy_work);
...
static int nf_tables_commit_chain_prepare(struct net *net,
					  struct nft_chain *chain)
{
	struct nft_rule *rule;
 ...
 */
static unsigned int
nf_tables_commit_audit_entrycount(const struct nft_trans *trans)
{
	switch (trans->msg_type) {
	case NFT_MSG_NEWSETELEM:
	case NFT_MSG_DELSETELEM:
...
static int nf_tables_commit(struct net *net, struct sk_buff *skb)
{
	...
	list_for_each_entry(trans_binding, &nft_net->binding_list,
			    binding_list) {
		...
		case NFT_MSG_NEWSET:
			if (!nft_trans_set_update(trans) &&
				...
				pr_warn_once(
					"nftables ruleset with unbound set\n");
				return -EINVAL;
			}
			break;
		case NFT_MSG_NEWCHAIN:
			if (!nft_trans_chain_update(trans) &&
				...
				pr_warn_once(
					"nftables ruleset with unbound chain\n");
				return -EINVAL;
			}
			break;
		default:
	...
	list_for_each_entry_safe(trans, next, &nft_net->commit_list, list) {
		...
		case NFT_MSG_NEWCHAIN:
			if (nft_trans_chain_update(trans)) {
				nft_chain_commit_update(
					nft_trans_container_chain(trans));
				nf_tables_chain_notify(
					&ctx, NFT_MSG_NEWCHAIN,
		...
		case NFT_MSG_NEWRULE:
			...
			nf_tables_rule_notify(&ctx, nft_trans_rule(trans),
					      NFT_MSG_NEWRULE);
			if (nft_trans_rule_chain(trans)->flags &
		...
		case NFT_MSG_DESTROYRULE:
			...
			break;
		case NFT_MSG_NEWSET:
			list_del(&nft_trans_container_set(trans)
					  ->list_trans_newset);
			if (nft_trans_set_update(trans)) {
				struct nft_set *set = nft_trans_set(trans);

				WRITE_ONCE(set->timeout,
					   nft_trans_set_timeout(trans));
				WRITE_ONCE(set->gc_int,
				 ...
				 */
				if (nft_set_is_anonymous(
					    nft_trans_set(trans)) &&
				    !list_empty(
					    &nft_trans_set(trans)->bindings))
		...
		case NFT_MSG_NEWFLOWTABLE:
			if (nft_trans_flowtable_update(trans)) {
				...
				nf_tables_flowtable_notify(
					&ctx, nft_trans_flowtable(trans), NULL,
					NFT_MSG_NEWFLOWTABLE);
			}
			nft_trans_destroy(trans);
			break;
...
static void nf_tables_abort_release(struct nft_trans *trans)
{
	...
	case NFT_MSG_NEWSET:
		...
		break;
	case NFT_MSG_NEWSETELEM:
		nft_trans_set_elem_destroy(&ctx,
					   nft_trans_container_elem(trans));
		break;
	case NFT_MSG_NEWOBJ:
		nft_obj_destroy(&ctx, nft_trans_obj(trans));
...
static int __nf_tables_abort(struct net *net, enum nfnl_abort_action action)
{
	...
	if (action == NFNL_ABORT_VALIDATE && nf_tables_validate(net) < 0)
		err = -EAGAIN;
...
static void nft_saw_register_store(const struct nft_ctx *__ctx, int reg,
				   unsigned int len)
{
				       ...
				       unsigned int len)
{
	...
	case NFT_REG_VERDICT:
		...
		if (data != NULL && (data->verdict.code == NFT_GOTO ||
...
int nft_verdict_dump(struct sk_buff *skb, int type, const struct nft_verdict *v)
{
	...
	case NFT_JUMP:
	case NFT_GOTO:
		if (nla_put_string(skb, NFTA_VERDICT_CHAIN, v->chain->name))
			goto nla_put_failure;
	}
	nla_nest_end(skb, nest);
	return 0;
...
again:
	list_for_each_entry(table, &nft_net->tables, list) {
		if (nft_table_has_owner(table) && n->portid == table->nlpid) {
...
static int __init nf_tables_module_init(void)
{
	int err;
#ifdef CONFIG_SAL_GENERAL
	pr_info("Loading self adjusting list");
#else
	pr_info("Loading default nf tables");
#endif

	BUILD_BUG_ON(offsetof(struct nft_trans_table, nft_trans) != 0);
	BUILD_BUG_ON(offsetof(struct nft_trans_chain,
			      nft_trans_binding.nft_trans) != 0);
	BUILD_BUG_ON(offsetof(struct nft_trans_rule, nft_trans) != 0);
	BUILD_BUG_ON(offsetof(struct nft_trans_set,
			      nft_trans_binding.nft_trans) != 0);
